...
export class TeamService {
  ...
  async list(query: ListTeamDto) {
    ...
    const teams = await this.prisma.ms_teams.findMany({
      where,
      orderBy: { name: 'asc' },
      include: {
  ...
  async create(userRole: string, dto: CreateTeamDto) {
    if (!this.canManageTeams(userRole)) {
  ...
  async update(userRole: string, id: string, dto: UpdateTeamDto) {
    if (!this.canManageTeams(userRole)) {
      throw new ForbiddenException(
        'Only manager HRGA, HRD, or admin can manage teams',
      );
