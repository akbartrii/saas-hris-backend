...
export class WorkScheduleService {
  ...
  async list() {
    return this.prisma.ms_work_schedules.findMany({ orderBy: { name: 'asc' } });
  }

  async create(userRole: string, dto: CreateWorkScheduleDto) {
  ...
  async update(userRole: string, id: string, dto: UpdateWorkScheduleDto) {
    if (!this.isAdminOrHRD(userRole)) {
      throw new ForbiddenException(
        'Only HRD or admin can manage work schedules',
      );
  ...
  async delete(userRole: string, id: string) {
    ...
    if (!exists) throw new NotFoundException('Work schedule not found');
    return this.prisma.ms_work_schedules.delete({ where: { id } });
  }
}
