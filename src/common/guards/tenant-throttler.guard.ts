...
export class TenantThrottlerGuard extends ThrottlerGuard {
  ...
  private async resolveLimitValue(
    context: ExecutionContext,
    value: number | ((ctx: ExecutionContext) => number | Promise<number>),
  ): Promise<number> {
    return typeof value === "function"
      ? (value as (ctx: ExecutionContext) => number | Promise<number>)(context)
      : value;
  }

  private async resolvePlanLimit(
    context: ExecutionContext,
    ...
    try {
      ...
      if (subscription?.plan) {
        ...
        if (planLimits) {
          const planLimit =
            planLimits[throttlerName as keyof typeof planLimits];
  ...
  async canActivate(context: ExecutionContext): Promise<boolean> {
    ...
    for (const namedThrottler of this.throttlers) {
      ...
      const skip = this.reflector.getAllAndOverride(THROTTLER_SKIP + name, [
        handler,
        classRef,
      ]);
      ...
      let limit = await this.resolveLimitValue(
        context,
        routeOrClassLimit || namedThrottler.limit,
      );
      const ttl = await this.resolveLimitValue(
        context,
        routeOrClassTtl || namedThrottler.ttl,
      );
      const blockDuration = await this.resolveLimitValue(
        context,
