...
describe("RemoteWorkService", () => {
  ...
  describe("list", () => {
    it("should return paginated requests", async () => {
      ...
      mockPrisma.tr_remote_work_requests.findMany.mockResolvedValue([
        { id: "rw1", status: "pending" },
      ]);
  ...
  describe("create", () => {
    it("should create a request", async () => {
      ...
      mockPrisma.tr_remote_work_requests.create.mockResolvedValue({
        id: "rw1",
        reason: "WFH",
      });
  ...
  describe("approve", () => {
    it("should approve a pending request", async () => {
      mockPrisma.tr_remote_work_requests.findUnique.mockResolvedValue({
        id: "rw1",
        company_id: "c1",
        employee_id: "e1",
        status: "pending",
      });
      mockPrisma.tr_remote_work_requests.update.mockResolvedValue({
        id: "rw1",
        status: "approved",
      });

      const result = await service.approve("u1", "c1", "admin", "rw1", {
        action: "approve",
      } as any);
