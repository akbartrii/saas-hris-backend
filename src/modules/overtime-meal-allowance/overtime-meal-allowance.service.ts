...
export class OvertimeMealAllowanceService {
  ...
  async create(companyId: string, userRole: string, dto: CreateOvertimeMealAllowanceDto) {
    if (!this.isAdminOrHRD(userRole)) {
      throw new ForbiddenException('Only HRD or admin can manage overtime meal allowances');
    }
  ...
  async update(
    ...
    if (!exists) throw new NotFoundException('Overtime meal allowance not found');
    const data: any = {};
    if (dto.day_type) data.day_type = dto.day_type;
    if (dto.time_start) data.time_start = new Date('1970-01-01T' + dto.time_start);
  ...
  async delete(companyId: string, userRole: string, id: string) {
    ...
    if (!exists) throw new NotFoundException('Overtime meal allowance not found');
    return this.prisma.ms_overtime_meal_allowances.delete({ where: { id, company_id: companyId } });
  }
}
