...
describe("RecruitmentService", () => {
  ...
  describe("createJob", () => {
    ...
    it("should throw ForbiddenException for non-admin role", async () => {
      mockPrisma.ms_users.findUnique.mockResolvedValue({
        ...mockUser,
        ms_roles: { name: "karyawan" },
      });
      await expect(service.createJob("u1", "c1", dto as any)).rejects.toThrow(
        ForbiddenException,
      );
    });
    ...
    it("should create a job posting", async () => {
      ...
      mockPrisma.ms_job_postings.create.mockResolvedValue({
        id: "j1",
        title: dto.title,
        status: "active",
        public_slug: "software-engineer-123",
      });
    ...
    it("should throw if slug already exists", async () => {
      mockPrisma.ms_users.findUnique.mockResolvedValue(mockUser);
      mockPrisma.ms_job_postings.findUnique.mockResolvedValue({
        id: "existing",
      });

      await expect(
        service.createJob("u1", "c1", { ...dto, public_slug: "taken" } as any),
  ...
  describe("listJobs", () => {
    it("should return paginated jobs", async () => {
      mockPrisma.ms_job_postings.findMany.mockResolvedValue([
        { id: "j1", title: "Engineer" },
      ]);
      mockPrisma.ms_job_postings.count.mockResolvedValue(1);

      const result = await service.listJobs("c1", {
        page: 1,
        limit: 10,
      } as any);
  ...
  describe("applyJob", () => {
    ...
    it("should throw if job not found", async () => {
      mockPrisma.ms_job_postings.findUnique.mockResolvedValue(null);
      await expect(service.applyJob(dto as any)).rejects.toThrow(
        NotFoundException,
      );
    });
    ...
    it("should create application for active job", async () => {
      ...
      mockPrisma.tr_job_applications.create.mockResolvedValue({
        id: "a1",
        status: "new",
      });
  ...
  describe("updateApplicationStatus", () => {
    it("should update application status", async () => {
      mockPrisma.tr_job_applications.findUnique.mockResolvedValue({
        id: "a1",
        company_id: "c1",
        ms_job_postings: { title: "Engineer" },
      });
      mockPrisma.tr_job_applications.update.mockResolvedValue({
        id: "a1",
        status: "approved",
      });

      const result = await service.updateApplicationStatus("u1", "c1", "a1", {
        status: "approved",
        notes: "Hired",
      });
      expect(result).toBeDefined();
