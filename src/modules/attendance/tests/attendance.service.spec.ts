...
describe("AttendanceService", () => {
  ...
  describe("listAttendance", () => {
    it("should return paginated attendance", async () => {
      mockPrisma.tr_attendances.findMany.mockResolvedValue([
        { id: "a1", status: "present" },
      ]);
      mockPrisma.tr_attendances.count.mockResolvedValue(1);

      const result = await service.listAttendance("u1", "e1", "c1", {
        page: 1,
        limit: 10,
      } as any);
  ...
  describe("createCorrection", () => {
    ...
    it("should throw when attendance not found", async () => {
      mockPrisma.tr_attendances.findFirst.mockResolvedValue(null);
      await expect(
        service.createCorrection("u1", "e1", dto as any),
      ).rejects.toThrow(NotFoundException);
    });

    it("should create correction", async () => {
      mockPrisma.tr_attendances.findFirst.mockResolvedValue({
        id: "a1",
        company_id: "c1",
      });
      mockPrisma.tr_attendance_corrections.findFirst.mockResolvedValue(null);
      mockPrisma.tr_attendance_corrections.create.mockResolvedValue({
        id: "c1",
        status: "pending",
      });
  ...
  describe("cancelCorrection", () => {
    it("should throw when not found", async () => {
      ...
      await expect(service.cancelCorrection("e1", "c1")).rejects.toThrow(
        NotFoundException,
      );
    });

    it("should throw when not owner", async () => {
      mockPrisma.tr_attendance_corrections.findUnique.mockResolvedValue({
        id: "c1",
        employee_id: "other-e1",
      });
      await expect(service.cancelCorrection("e1", "c1")).rejects.toThrow(
        ForbiddenException,
      );
    ...
    it("should cancel pending correction", async () => {
      ...
      mockPrisma.tr_attendance_corrections.update.mockResolvedValue({
        id: "c1",
        status: "cancelled",
      });
  ...
  describe("listCorrections", () => {
    it("should return paginated corrections", async () => {
      mockPrisma.tr_attendance_corrections.findMany.mockResolvedValue([
        { id: "c1", status: "pending" },
      ]);
      mockPrisma.tr_attendance_corrections.count.mockResolvedValue(1);

      const result = await service.listCorrections("e1", {
        page: "1",
        limit: "10",
      });
      expect(result.data).toHaveLength(1);
