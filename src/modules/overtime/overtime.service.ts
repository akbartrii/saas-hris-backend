...
export class OvertimeService {
  ...
  private async calculateMealAllowance(
    ...
    const allowances = await this.prisma.ms_overtime_meal_allowances.findMany({
      where: { day_type: dayType, company_id: companyId },
    });
  ...
  async cancelOvertime(userId: string, companyId: string, overtimeId: string) {
  ...
  async getOvertimeSummary(userId: string, companyId: string, month?: string) {
    const user = await this.prisma.ms_users.findUnique({
      where: { id: userId },
      include: { ms_employees: true, ms_roles: true },
    });
