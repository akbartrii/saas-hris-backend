...
describe('TimeOffTypeService', () => {
  ...
  describe('list', () => {
    it('should return time-off types', async () => {
      mockPrisma.ms_time_off_types.findMany.mockResolvedValue([{ id: 't1', name: 'Sick Leave' }]);
  ...
  describe('create', () => {
    it('should create for admin', async () => {
      mockPrisma.ms_time_off_types.create.mockResolvedValue({ id: 't1', name: 'Sick Leave' });

      const result = await service.create('c1', 'admin', { name: 'Sick Leave' } as any);
