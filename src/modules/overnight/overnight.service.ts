...
export class OvernightService {
  ...
  async list(userId: string, companyId: string, userRole: string, query: ListOvernightDto) {
  ...
  async listSubordinateOvernights(userId: string, companyId: string, query: ListOvernightDto) {
    const user = await this.prisma.ms_users.findUnique({
      where: { id: userId },
      include: { ms_employees: true },
    });
  ...
  async create(userId: string, companyId: string, dto: CreateOvernightDto) {
    const user = await this.prisma.ms_users.findUnique({
      where: { id: userId },
      include: { ms_employees: true },
    });
  ...
  async approve(
    ...
    const isHR = ['manager_hrga', 'hrd', 'admin', 'super_admin'].includes(userRole);
