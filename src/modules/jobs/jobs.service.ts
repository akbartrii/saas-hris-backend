...
export class JobsService {
  ...
  async dispatchPayrollGeneration(dto: PayrollGeneratorDto) {
    ...
    this.logger.log(`Dispatched payroll generation job ${job.id} for period ${dto.periodId}`);
    return { jobId: job.id };
  }
  ...
  async dispatchExport(dto: ExportDto) {
    ...
    this.logger.log(`Dispatched export job ${job.id}: ${dto.type} as ${dto.format}`);
    return { jobId: job.id };
  }
  ...
  async dispatchNotificationFanout(dto: NotificationFanoutDto) {
    ...
    this.logger.log(`Dispatched notification fanout job ${job.id} to ${dto.userIds.length} users`);
    return { jobId: job.id };
  }
  ...
  async dispatchUsageTracking(dto: UsageTrackingDto) {
    ...
    this.logger.log(`Dispatched usage tracking job ${job.id} for company ${dto.companyId}`);
    return { jobId: job.id };
  }
