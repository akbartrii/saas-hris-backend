...
export class WorkScheduleService {
  ...
  async create(companyId: string, userRole: string, dto: CreateWorkScheduleDto) {
    if (!this.isAdminOrHRD(userRole)) {
      throw new ForbiddenException('Only HRD or admin can manage work schedules');
    }
    const data: any = { ...dto, company_id: companyId };
    if (dto.start_time) data.start_time = new Date(`1970-01-01T${dto.start_time}`);
    if (dto.end_time) data.end_time = new Date(`1970-01-01T${dto.end_time}`);
    if (dto.break_start) data.break_start = new Date(`1970-01-01T${dto.break_start}`);
    if (dto.break_end) data.break_end = new Date(`1970-01-01T${dto.break_end}`);
    return this.prisma.ms_work_schedules.create({ data });
  }

  async update(companyId: string, userRole: string, id: string, dto: UpdateWorkScheduleDto) {
    if (!this.isAdminOrHRD(userRole)) {
      throw new ForbiddenException('Only HRD or admin can manage work schedules');
    }
  ...
  async delete(companyId: string, userRole: string, id: string) {
    ...
    if (!exists) throw new NotFoundException('Work schedule not found');
    return this.prisma.ms_work_schedules.delete({ where: { id, company_id: companyId } });
  }
}
