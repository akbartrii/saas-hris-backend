...
describe("CompanyService", () => {
  ...
  describe("list", () => {
    it("should return companies", async () => {
      mockPrisma.ms_companies.findMany.mockResolvedValue([
        { id: "c1", name: "Test Corp" },
      ]);
  ...
  describe("create", () => {
    it("should create for super_admin", async () => {
      mockPrisma.ms_companies.create.mockResolvedValue({
        id: "c1",
        name: "New Corp",
      });

      const result = await service.create("super_admin", "admin-id", {
        name: "New Corp",
      } as any);
  ...
  describe("update", () => {
    it("should update for super_admin", async () => {
      ...
      mockPrisma.ms_companies.update.mockResolvedValue({
        id: "c1",
        name: "Updated Corp",
      });

      const result = await service.update("super_admin", "c1", "c1", {
        name: "Updated Corp",
      } as any);
    ...
    it("should throw ForbiddenException for non-super_admin", async () => {
      await expect(
        service.update("karyawan", "c1", "c1", {} as any),
