...
describe('HolidayCalendarService', () => {
  ...
  describe('list', () => {
    it('should return holidays list', async () => {
      ...
      mockPrisma.ms_holiday_calendars.findMany.mockResolvedValue([{ id: 'h1', name: 'New Year' }]);
  ...
  describe('create', () => {
    it('should create a holiday for admin', async () => {
      ...
      mockPrisma.ms_holiday_calendars.create.mockResolvedValue({ id: 'h1', name: 'Independence Day' });

      const result = await service.create('u1', 'c1', 'admin', { name: 'Independence Day' } as any);

      expect(result.name).toBe('Independence Day');
    });

    it('should throw ForbiddenException for non-admin', async () => {
      await expect(service.create('u1', 'c1', 'karyawan', {} as any)).rejects.toThrow(ForbiddenException);
    });
  });

  describe('delete', () => {
    it('should delete a holiday for admin', async () => {
      mockPrisma.ms_holiday_calendars.findUnique.mockResolvedValue({ id: 'h1', company_id: 'c1' });
    ...
    it('should throw ForbiddenException for non-admin', async () => {
      await expect(service.delete('u1', 'c1', 'karyawan', 'h1')).rejects.toThrow(ForbiddenException);
    });
  });
});
