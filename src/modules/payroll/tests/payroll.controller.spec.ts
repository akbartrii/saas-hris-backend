...
describe("PayrollController", () => {
  ...
  describe("listPayslips", () => {
    it("should delegate to service", async () => {
      ...
      const result = await controller.listPayslips(
        "u1",
        "c1",
        "admin",
        {} as any,
      ...
      expect(payrollService.listPayslips).toHaveBeenCalledWith(
        "u1",
        "c1",
        {},
        "admin",
  ...
  describe("getPayslipDetail", () => {
    it("should delegate to service", async () => {
      ...
      const _result = await controller.getPayslipDetail(
        "u1",
        "c1",
        "admin",
        "ps1",
      ...
      expect(payrollService.getPayslipDetail).toHaveBeenCalledWith(
        "u1",
        "c1",
        "ps1",
        "admin",
  ...
  describe("generateBatchPayslip", () => {
    it("should delegate to service with keycode", async () => {
      ...
      const result = await controller.generateBatchPayslip(
        "u1",
        "c1",
        "admin",
        "valid-key",
  ...
  describe("createPayrollPeriod", () => {
    it("should delegate to service", async () => {
      mockPayrollService.createPeriod.mockResolvedValue({
        id: "p1",
        period_name: "July 2026",
      });

      const _result = await controller.createPayrollPeriod(
        "c1",
        { month: 7, year: 2026 } as any,
        "admin",
      );
